...
def test_a_parallel_turn_past_the_budget_is_refused_not_rendered():
    ...
    # turn used to render and send all three crops and only then notice it had
    # reached eight. Codex review of #131.
    executed: list[str] = []

    def responder(kw, _n):
        if not _tool_result_turns(kw):
            # Turn 1: five single requests would take five turns, so ask for
            # five at once — spending exactly five of the six.
            return FakeMessage(
                content=[
                    FakeToolUseBlock(name="find_text", input={"query": f"Q{i}"},
                                     id=f"a{i}")
                    for i in range(5)
                ],
                stop_reason="tool_use", usage=FakeUsage(),
            )
        if not _tools_callable(kw):
            return _verdict("NOT_VISIBLE", "could not settle")
        # Turn 2: three more, but only ONE request of budget is left.
        return FakeMessage(
            content=[
                FakeToolUseBlock(name="find_text", input={"query": f"R{i}"},
                                 id=f"b{i}")
                for i in range(3)
            ],
            stop_reason="tool_use", usage=FakeUsage(),
    ...
    # Exactly the budget was spent — never 5 + 3.
    assert f.verification.investigation_rounds == 6

    # Every id in that turn was still answered, in ONE user turn (the API
    # requires it), with the excess refused rather than executed.
    turns = [
        m["content"] for m in client.calls[-1]["messages"]
        if m.get("role") == "user" and isinstance(m.get("content"), list)
        and any(isinstance(b, dict) and b.get("tool_use_id", "").startswith("b")
                for b in m["content"])
    ]
    assert len(turns) == 1, "the three ids must be answered in ONE user turn"
    (turn,) = turns
    answered = [b["tool_use_id"] for b in turn
                if isinstance(b, dict) and b.get("type") == "tool_result"]
    assert answered == ["b0", "b1", "b2"]
    refused = [b for b in turn
               if isinstance(b, dict) and b.get("content") == inv._BUDGET_REFUSED_TEXT]
    assert [b["tool_use_id"] for b in refused] == ["b1", "b2"]
    assert all(b.get("is_error") for b in refused)


def test_a_refused_request_renders_no_evidence(tmp_path):
    # The point of the cap is the cost: a refused block must produce no crop on
    # disk and no tool-trace entry, or the budget bounds only the arithmetic.
    def responder(kw, _n):
        if not _tool_result_turns(kw):
            return FakeMessage(
                content=[
                    FakeToolUseBlock(name="crop_region",
                                     input={"rect": [10, 10, 300, 200]}, id="c1"),
                    FakeToolUseBlock(name="crop_region",
                                     input={"rect": [20, 20, 400, 300]}, id="c2"),
                    FakeToolUseBlock(name="crop_region",
                                     input={"rect": [30, 30, 500, 400]}, id="c3"),
                ],
                stop_reason="tool_use", usage=FakeUsage(),
            )
        return _verdict("NOT_VISIBLE", "could not settle")
    ...
    client = _LoopClient(responder)
    _res, f = _run_one(client, max_rounds=1, evidence_dir=tmp_path)

    assert f.verification.investigation_rounds == 1

    # One granted crop reached the model as an image; the other two came back
    # as costless refusals.
    turn = next(
        m["content"] for m in client.calls[-1]["messages"]
        if m.get("role") == "user" and isinstance(m.get("content"), list)
    ...
    results = [b for b in turn if isinstance(b, dict) and b.get("type") == "tool_result"]
    assert [b["tool_use_id"] for b in results] == ["c1", "c2", "c3"]
    with_image = [
        b for b in results
        if isinstance(b.get("content"), list)
        and any(part.get("type") == "image" for part in b["content"])
    ]
    assert [b["tool_use_id"] for b in with_image] == ["c1"]
    assert all(b.get("content") == inv._BUDGET_REFUSED_TEXT for b in results[1:])

    # And nothing was written for them. Two files, not four: the loop's own
    # opening evidence crop of the finding, plus the single granted crop.
    saved = [p for p in tmp_path.rglob("*.png") if p.is_file()]
