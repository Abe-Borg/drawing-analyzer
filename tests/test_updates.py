"""Hermetic tests for the self-update checker (``core/updates.py``).

No network, no clock dependence: the manifest fetcher, the download opener, and
the throttle clock (``now=``) are all injected (I-4). Covers version ordering
...
filename safety, and the once-a-day / skip-version throttle.
"""
...
from drawing_analyzer.core.updates import (
    STATUS_DISABLED,
    STATUS_ERROR,
    STATUS_UP_TO_DATE,
...
def _manifest(**overrides) -> dict:
    payload = {
        "version": "2.0.0",
        "url": "https://github.com/abe-borg/drawing-analyzer/releases/download/v2.0.0/DrawingAnalyzerSetup.exe",
        "sha256": _GOOD_SHA,
...
def test_disable_off_tokens_keep_checking(monkeypatch, token: str) -> None:
    monkeypatch.setenv("DRAWING_ANALYZER_DISABLE_UPDATE_CHECK", token)
...
def test_check_passes_manifest_url(monkeypatch) -> None:
    ...
    monkeypatch.setenv("DRAWING_ANALYZER_UPDATE_URL", "https://example.test/latest.json")
...
@pytest.mark.parametrize(
    "url, expected",
    [
        ("https://host/DrawingAnalyzerSetup.exe", "DrawingAnalyzerSetup.exe"),
        ("https://host/path/to/Custom.exe", "Custom.exe"),
        ("https://host/DrawingAnalyzerSetup.exe?token=abc", "DrawingAnalyzerSetup.exe"),
        ("https://host/no-extension", "DrawingAnalyzerSetup.exe"),      # fallback
        ("https://host/../../evil.exe", "evil.exe"),                    # traversal stripped
        ("https://host/", "DrawingAnalyzerSetup.exe"),
    ],
)
...
def test_make_manifest_script_roundtrips(tmp_path: Path) -> None:
    ...
    installer = tmp_path / "DrawingAnalyzerSetup.exe"
    installer.write_bytes(b"installer payload")
    out = tmp_path / "latest.json"
    module.write_manifest(
        version="1.2.3",
        installer=installer,
        url="https://github.com/abe-borg/drawing-analyzer/releases/download/v1.2.3/DrawingAnalyzerSetup.exe",
        out_path=out,
        notes="Release 1.2.3",
        published_at="2026-07-17",
    )
