...
| P5 — batch harvest + 24h bound (items 14a, 14b) | **none** | nothing. The harvest reads a batch's *existing* results through the same `_parse_item` path a terminal collect uses, and writes cache entries with the same keys it always would — a harvested sheet is byte-identical to one collected normally. The bound and its `DRAWING_ANALYZER_BATCH_MAX_ELAPSED_HOURS` override are wall-clock policy, invisible to every key: no prompt text, no stored shape, no model-visible string moved. `DETACHED_MOVING` appears only in log lines, per-sheet error text and non-billable `ABANDONED_*` ledger records, none of which are hashed. |
| P5 — investigation evidence budget (item 17) | `investigate.INVESTIGATE_PROMPT_VERSION` **`investigate-v2` → `investigate-v3`** | concluded investigation verdicts only. No prompt STRING moved — the budget sentence already read "up to N evidence request(s)" — but the counter now spends per request rather than per turn, and ``round_budget`` is a cache-key input, so a verdict stored under turn-counting is not reproducible under the same key. The version string is hand-maintained and is the only mechanism covering this stage; a **separate mechanism for a separate change** from the P5 batch work below, which needs none. Nothing else in P5 touches a key: the usage predicate, the citation cache-token record, the refusal-fallback registry gate, the diagnostics blob filter and the cost wording are all outside every hashed input. |
| P6 — report chat turn lifecycle | **none** | nothing. Every change is inside the report's embedded JS (turn-generation guards, the Stop latch, the dangling-`tool_use` strip, the note's owner) or is host-side request shape resolved from the capability registry (`thinking` / `webSearch` handed to `CFG` like `webFetch` already was). No prompt text, no stored shape, no hashed key input moves; the widget runs on the reader's own key and its spend never enters the run ledger. The new `ModelCapabilities.supports_web_search` is a request-shape fact, read at build time, never hashed. |
| P7 — reviewed-PDF geometry and markup | `render._RENDER_IDENTITY_SCHEME` **`render-identity-v3` → `v4`** (item 28 only) | level-1 digest and critique entries, once — one re-render per sheet. A *policy* change, not a document change: `_page_dependency_sha256` hashes the annotation **bytes**, and those bytes did not move, so without the bump an affected page still hits level 1 and is served a digest built from annotation-contaminated `sheet_text` **without re-extracting the text** — a false hit, the one failure mode the identity exists to prevent. Deliberately the only mechanism for it: no `text_annots` term was added to the identity's parts list, since a new term would move every key by itself and be a second mechanism for one change. Level-2 keys move on their own for affected pages (`sheet_text` and the PNG bytes are both key inputs, and `is_raster` changes `target_px`), and `_cross_qc_cache_key` moves on its own too, so `_CROSS_QC_CACHE_CONTRACT` was **not** bumped. Confirmed by `benchmark_drawing_analyzer.py --check`: `standard-warm` still 0 digest calls and 0 renders, `one-source-changed` still exactly 1. |
| P7 — page-local render identity (N23) | **none** | nothing, and one fewer false miss. Treating another `/Type /Page` object as an opaque leaf makes an affected page's identity **change once** (it re-keys and re-renders), so there is no stale entry to serve; unaffected pages are byte-identical. Not a false-hit risk in the other direction either: a GOTO target cannot change this page's pixels, and the destination reference still rides the hash inside the referencing annot object, so retargeting the link moves the key — asserted, together with the edited page's own identity still moving. |
| P7 — everything else (items 27, 29–34, N24) | **none** | nothing. The destination transforms, the fuzzy numeric veto, the occupancy sampler, the clear-band computation, the FreeText truncation, the index column fitting, the non-PDF rejection and the placement labels all live downstream of every hashed input: no prompt text, no stored shape, no model-visible string and no key input moves. The anchor veto changes which findings anchor, but anchoring runs per-run after the critique stage and its output is never cached. |
| P8 — text handling and sanitizers | `cross_qc._CROSS_QC_CACHE_CONTRACT` **2 → 3** (item 11 only) | cross-QC stage entries, once. `_norm_id` now folds sheet handles through `auditors.sheet_ids.fold_text`, so a leg whose handle carried a Unicode dash, a U+2010 hyphen or fullwidth digits resolves where it used to be dropped. That is **host-side binding**, not a model input — nothing in the key covers it — yet it changes which legs validate and therefore the stored result for byte-identical request inputs, so a warm entry would keep serving the smaller finding set forever. Deliberately the only mechanism: no key field was added beside it. The `critique._leg_targets` twin moves with it and needs none of its own (that value feeds `critical_signature`, which is recomputed per run and never stored under a key of its own). |
| P8 — everything else (items 6, 9, 12, 39, N5, N7–N10) | **none** | nothing. The `_as_list` coercion, the prose boilerplate/length filter and its new `filtered` counter, the case-sensitive code-designation pattern, the NUL strip in the report renderer, the round-robin plan trim, the vulgar-fraction fold, the invisible-character escapes, the spec-document decode/merged-cell/running-head handling and the diagnostics redaction all sit outside every hashed input: no prompt text, no stored shape, no model-visible string, no key input moves. The identity and review-plan stages cache on the *corpus and prompt* they send, which are unchanged — a sanitizer runs on the reply after the key is computed. The anchor fold changes which findings anchor, but anchoring runs per-run after the critique stage and its output is never cached. Confirmed by `benchmark_drawing_analyzer.py --check`: `standard-warm` still 0 digest calls and 0 renders, `one-source-changed` still exactly 1. |
| P9 — Windows, filesystem, CI, supply chain, docs | **none** | nothing. Every change sits outside every hashed input. The sanitizer and redaction work (items 44) rewrites *artifact text* — run.log, run_manifest.json, the exported Markdown and the HTML report — all of which are produced after the run and hashed by nobody; the digest prose those files carry is byte-identical (I-2), asserted directly. The casefold dedupe (45) renames export files, not cache keys. The long-path form and the publish backoff (N30) are I/O policy; the prefixed path never enters a key, and `long_path` is an identity function off Windows. The work-dir pruning (46) deletes *evidence crops and temporary renders* under `%TEMP%`, never a `DigestCache` entry — the cache lives in the app data dir and a pruned work dir costs a re-render, not a re-request (pruning is age-based and skips the live run's own dir). The GUI guards (47, N32), the workflow gates (40, 41, 42) and the documentation corrections (N28, N29, N31) touch no runtime path a key covers. No prompt text, no stored shape, no model-visible string moved, so no counter was bumped — and none should be: a bump here would discard every warm entry to no purpose. |
