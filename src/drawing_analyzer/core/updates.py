...
Drawing Analyzer ships as a downloadable Windows app (see ``packaging/windows/``
...
installer:
    ...
    1. Each published release carries a tiny ``latest.json`` manifest and the
       installer (``DrawingAnalyzerSetup.exe``) as release assets.
...
- **Pure and self-contained.** This module imports only the standard library
  (plus the sibling :mod:`app_paths`). It does NOT import the heavy parent
  package, ``customtkinter``, or PyMuPDF, so it stays trivially unit-testable
  (I-4) and outside the PyMuPDF isolation boundary (I-5). The *caller* supplies
  the current version string — the update logic never reaches into the package
  to find it.
- **Non-fatal, like every QC stage (I-3 spirit).** :func:`check_for_update`
  never raises: any network / parse / comparison failure is folded into an
  ``ERROR`` result the GUI can quietly ignore (silent auto-check) or surface
  (explicit "Check for Updates" click).
- **Injectable seams for hermetic tests.** The network fetcher, the download
  ...
  flow without touching the network (I-4).

Env overrides (matching the ``DRAWING_ANALYZER_*`` convention in
:mod:`api_config`):

    DRAWING_ANALYZER_UPDATE_URL           — override the manifest URL (testing /
                                            self-hosting a fork's releases).
    DRAWING_ANALYZER_DISABLE_UPDATE_CHECK — set truthy to turn the check off
...
ENV_UPDATE_URL = "DRAWING_ANALYZER_UPDATE_URL"
ENV_DISABLE = "DRAWING_ANALYZER_DISABLE_UPDATE_CHECK"

# Mirrors the disable-token convention used across the codebase (api_config,
# diagnostics): any of these, case-insensitively, reads as "off".
...
_USER_AGENT = "DrawingAnalyzer-Updater"
...
_INSTALLER_FALLBACK_NAME = "DrawingAnalyzerSetup.exe"
...
def manifest_url() -> str:
    """The manifest URL: the ``DRAWING_ANALYZER_UPDATE_URL`` override or default."""
...
def update_check_disabled() -> bool:
    """Whether update checks are turned off via ``DRAWING_ANALYZER_DISABLE_UPDATE_CHECK``."""
...
def check_for_update(
    ...
    *,
    ...
    :func:`fetch_manifest`; tests inject a fake to stay hermetic (I-4).
    """
...
def default_state_path() -> Path:
    ...
    return app_config_dir() / STATE_FILENAME
...
def should_auto_check(
    ...
    *,
    ...
    (mirrors the injectable-clock discipline in I-7). A missing or unparseable
    ``last_check`` means "never checked" → check now.
    """
