...
- **Punctuation, done by WP-05.2 (decided with the owner):** after `_normalize`, every word (of the sheet and of the quote alike) loses the brackets and sentence punctuation at its edges (`anchor.fold_word`: leading `( [ {`, trailing `) ] } , ; : . ! ?`; the `word_core` edge sets minus `"` `'` `<` `>`, pinned). Never folded: `"` and `'` (inch and foot marks), `<` and `>` (comparisons), `%`, `/`, `-`, a leading `.`. A word that folds to nothing (a lone comma) takes no part. One matcher (`SourceWords`, built from a string or from a sheet's words, keeping each word's index, so the mapping to the original word rectangles is retained) serves cross-QC and the anchor's EXACT and sub-phrase tiers, so the two agree; cross-QC's admission changed, so its contract went 4 → 5. Not taken: also folding `<` and `>` (a comparison sign would become cosmetic inside a span). A fold on the sheet side only would break a verbatim quote that carries the punctuation. "Extraction spacing" (a mark extracted as its own word, merged words, a split dimension) is WP-05.3's.
