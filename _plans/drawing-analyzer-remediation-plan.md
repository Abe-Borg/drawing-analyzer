...
- **Punctuation, done by WP-05.2 (decided with the owner):** after `_normalize`, every word (of the sheet and of the quote alike) loses the brackets and sentence punctuation at its edges (`anchor.fold_word`: leading `( [ {`, trailing `) ] } , ; : . ! ?`; the `word_core` edge sets minus `"` `'` `<` `>`, pinned). Never folded: `"` and `'` (inch and foot marks), `<` and `>` (comparisons), `%`, `/`, `-`, a leading `.`. A word that folds to nothing (a lone comma) takes no part. One matcher (`SourceWords`, built from a string or from a sheet's words, keeping each word's index, so the mapping to the original word rectangles is retained) serves cross-QC and the anchor's EXACT and sub-phrase tiers, so the two agree; cross-QC's admission changed, so its contract went 4 → 5. The fact-tile join keys on the same folded form (`cross_qc._norm_for_match`; the Codex review of WP-05.2): keyed on `_normalize` alone, a leg that grounded only through the fold lost its fact's tile, its only location on a scanned sheet. Not taken: also folding `<` and `>` (a comparison sign would become cosmetic inside a span). A fold on the sheet side only would break a verbatim quote that carries the punctuation. "Extraction spacing" (a mark extracted as its own word, merged words, a split dimension) is WP-05.3's.
...
- **WP-05.2:** the veto compares the folded tokens (the fold is per word, on both sides), so `175,` against `175` no longer refuses a window; the quantity-aware comparison is still WP-05.3's. Recorded limit, present before for unbracketed text: the sub-phrase veto reads digit-bearing tokens only, so a sub-phrase can drop a unit printed as its own word (`150 GPM 568 L/S` anchors FUZZY on `150 GPM (568`), which the fold extends to a bracketed number.
...
- **Done for the anchor by WP-05.2 (decided with the owner):** every tier matches whole source words. EXACT and the sub-phrase tier match through `SourceWords` (the same `word_core`, and the matcher cross-QC grounds through); a fuzzy window must start on a word's first token and end on one's last (`_Stream.on_word_edges`); and inside a window a quote's measurement may not match part of a sheet word (`anchor._measurements_whole`: every digit-bearing token of a word a measurement aligned to must be aligned too). **Correction, measured:** a boundary on EXACT alone fixes nothing, because every match EXACT refuses for cutting a word falls through to the window at 100% overlap (`VAV-2` → FUZZY on `VAV-2-1`); window edges alone miss a long note that leaves out one quote word and takes in the longer tag's extra number (its edges are whole words), which only the word veto refuses. Not taken: a veto refusing any digit the window adds (it also refused a window whose sheet prints a number the quote left out, a `(689 KPA)` conversion, and broke a pinned window test). Accepted limit: the word veto reads numbers, so a letter-only tag (`VAV-A` inside `VAV-A-1`) in a long window still anchors FUZZY. A folded match keeps `EXACT`/`exact`, so `numbers_grounded` holds.
...
- WP-05.2's fold changes which legs and facts cross-QC admits the same way: 4 → 5, with the same three tripwires updated. The anchor stage itself is not cached; a moved anchor re-keys that finding's verification and investigation entries (their keys carry the anchor), which is new paid work by design. The A/B `RECORD_CONTRACT_VERSION` stays 3 (a record stores the anchor as an arm output whose shape and meaning are unchanged).
