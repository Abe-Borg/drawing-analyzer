> **Frozen reference copy.** Copied unchanged, apart from this note, from
> `docs/DEEP_REVIEW_2026-09.md` on branch `claude/peaceful-goldberg-cs4kus` at
> commit `be3dbb8` (which was never merged into `main`). It is the "original
> report" that
> [`drawing-analyzer-remediation-plan.md`](drawing-analyzer-remediation-plan.md)
> cites by ID (`B*`, `R*`, `C*`, `$*`, `K*`, `A*`, `H*`, `G*`). Its `file:line`
> references are against `f5284ac` (1.6.0), so locate code by symbol name. Do
> not edit this file. Current status of every finding lives in
> [`PROGRESS.md`](PROGRESS.md); the 2026-09-22 re-verification against 1.7.0 is
> summarized in plan §1.3.
