...
installed app fetches to discover updates (see
``src/drawing_analyzer/core/updates.py`` — the ``parse_manifest`` there is the
consumer, and ``tests/test_updates.py`` round-trips this maker through it so the
two can never drift).

Usage:
    python packaging/windows/make_manifest.py \
        --version 1.2.3 \
        --installer dist/DrawingAnalyzerSetup.exe \
        --url https://github.com/OWNER/REPO/releases/download/v1.2.3/DrawingAnalyzerSetup.exe \
        --out latest.json \
